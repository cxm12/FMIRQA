...
def savecolorim_hist(save, im, **imshow_kwargs):
    ...
    # im: Uint8
    imshow_kwargs['cmap'] = 'magma'
    imshow_kwargs['vmin'] = im.min()
    imshow_kwargs['vmax'] = im.max()
    
    im = np.asarray(im)
    im = np.stack(map(to_color, im)) if 1 < im.shape[-1] <= 3 else im
    ndim_allowed = 2 + int(1 <= im.shape[-1] <= 3)
    proj_axis = tuple(range(1, 1 + max(0, im[0].ndim - ndim_allowed)))
    img = np.max(im, axis=proj_axis)
    # plt.hist(im, 256)
    ...
    # Apply histogram equalization using the cumulative distribution function
    img_equalized = np.interp(img.flatten(), bins[:-1], cdf_normalized).reshape(img.shape)
    
    # Save the equalized image to a file
    plt.imsave(save, img_equalized, cmap='magma')
