...
from result_IQA.model.swinir import *
...
class UniModel(nn.Module):
    def __init__(self, args, tsk=1, img_size=64, patch_size=1,
                 ...
                 use_checkpoint=False, num_feat=32, srscale=2, inchannel=5, outchannel=1):
        ...
        # 2 denoise
        self.conv_firstdT = nn.Conv2d(inchannel, embed_dim, 3, 1, 1)
        ...
        self.conv_last0 = nn.Conv2d(num_feat, outchannel, 3, 1, 1)
...
class UniModelSR(nn.Module):
    def __init__(self, args, tsk=1, out=0, img_size=64, patch_size=1,
                 embed_dim=180 // 2, depths=[6, 6, 6], num_heads=[6, 6, 6],
                 window_size=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, num_feat=32):
        super(UniModelSR, self).__init__()
        self.img_range = 1
        self.mean = torch.zeros(1, 1, 1, 1)
        self.window_size = window_size
        self.task = 1
        ...
        if self.out > 1:
            self.conv_before_upsample0 = nn.Sequential(nn.Conv2d(embed_dim, num_feat, 3, 1, 1),
...
class UniModeldenoise(nn.Module):
    def __init__(self, args, tsk=1, out=0, img_size=64, patch_size=1,
                 embed_dim=180 // 2, depths=[6, 6, 6], num_heads=[6, 6, 6],
                 window_size=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, num_feat=32):
        super(UniModeldenoise, self).__init__()
        self.img_range = 1
        self.mean = torch.zeros(1, 1, 1, 1)
        self.window_size = window_size
        self.task = 1
...
class UniModelISO(nn.Module):
    def __init__(self, args, tsk=1, out=0, img_size=64, patch_size=1,
                 ...
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, num_feat=32):
        super(UniModelISO, self).__init__()
        self.img_range = 1
        self.mean = torch.zeros(1, 1, 1, 1)
        self.window_size = window_size
        self.task = 1
...
class UniModelProj(nn.Module):
    def __init__(self, args, tsk=1, out=0, img_size=64, patch_size=1,
                 ...
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, num_feat=32):
        super(UniModelProj, self).__init__()
        self.img_range = 1
        self.mean = torch.zeros(1, 1, 1, 1)
        self.window_size = window_size
        self.task = 1
        ...
        args.n_feats = 256
        args.inch = 50
        self.project = Projhead(args=args, act=None)
        self.conv_firstproj = nn.Conv2d(1, embed_dim, 3, 1, 1)
...
class UniModelVolume(nn.Module):
    def __init__(self, args, tsk=1, out=0, img_size=64, patch_size=1,
                 embed_dim=180 // 2, depths=[6, 6, 6], num_heads=[6, 6, 6],
                 window_size=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, num_feat=32):
        super(UniModelVolume, self).__init__()
        self.img_range = 1
        self.mean = torch.zeros(1, 1, 1, 1)
        self.window_size = window_size
        self.task = 1
        ...
        if self.out > 1:
            self.conv_before_upsamplev = nn.Sequential(nn.Conv2d(embed_dim, embed_dim, 3, 1, 1),
                                                       nn.LeakyReLU(inplace=True))
            self.conv_lastv = nn.Conv2d(embed_dim, 61, 3, 1, 1)
